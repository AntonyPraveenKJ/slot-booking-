...
function ApprovedData() {
    ...
    const getApprovedData=async()=>{
        await axios.get("http://localhost:5000/adminRoute/getApproved").then((response)=>{
          setApproved(response.data.message)
        })
    };

    const handleApprovedDetail=async(id)=>{
        // console.log(id)
           await axios.get(`http://localhost:5000/adminRoute/showApprovedData/${id}`).then((response)=>{
        ...
        <TableBody>
            ...
            {approved.map((item)=>{
                return(
                    ...
                    <StyledTableCell align="right"><Button  variant="contained" onClick={()=>{handleApprovedDetail(item._id)}}>View</Button></StyledTableCell>
                  </StyledTableRow>
                )
            })}
